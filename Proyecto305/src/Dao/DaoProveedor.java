...
public class DaoProveedor {
	...
	public ArrayList<Proveedor> fetchProveedors(){
		...
		try {
			...
			while(rs.next()) {
				...
				u.setCantacto(rs.getInt("Cantacto"));
	...
	public boolean editarProveedor(Proveedor user) {
		PreparedStatement ps=null;
		try {
			ps=cx.conectar().prepareStatement("UPDATE Proveedor SET nombreproveedor=?, cantacto=?, descripcion=? WHERE idproveedor=?");
			ps.setString(1, user.getNombreproveedor());
			ps.setInt(2, user.getCantacto());
			ps.setString(3, user.getDescripcion());
			ps.setInt(5, user.getIdproveedor());
			ps.executeUpdate();
			return true;
		} catch (SQLException e) {
